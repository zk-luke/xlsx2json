...
function xlsx(file, options) { 
   ...
   if (typeof file === 'string') { 
   	// Load
		...
		if (s) {
           s = s.asText().split(/<si.*?>/g); i = s.length;//split with <si></si>, get all <t></t>
           while(--i) {
               var tArr = s[i].split(/<t.*?>/g);
               var tValue = '';
               tArr.forEach(function(e){
                   tValue += e.substring(0, e.indexOf('</t>'));
               })
               sharedStrings[i - 1] = unescapeXML(tValue);
		...
		while (i--) {
			...
			while (--j) { // Don't process j === 0, because s[0] is the text before the first row element
				...
				while (--k) { // Don't process l === 0, because k[0] is the text before the first c (cell) element
					...
					val = !val ?'':(t=='str') ? val : +val ; // turn non-zero into number   except t is str
					switch (t) {
