                        ...
                        case 'basic': // number string boolean
                            if (cell) {
                                if (cell.value instanceof Date) {
                                    var dateTime = moment(cell.value);
                                    if (dateTime.isValid()) {
                                       json_obj[col_name[i_col]] = dateTime.format("YYYY-MM-DD HH:mm:ss");
                                    }else{
 ...
 * parse object array.
 */
function parseObjectArrayField(row, key, cell) {

    var obj_array=[];

    //console.log(typeof(cell),cell,cell instanceof Date,key,row);

    if (typeof(cell) === 'string' && cell.indexOf(',') !== -1) {
        obj_array = cell.split(',');
    } else {
        obj_array.push(cell.toString());
    };

    var result = [];

    obj_array.forEach(function(element, index, value) {
        if (element) {
