...
module.exports = {
     ...
     * head 表头从第几行开始
     */
    toJson: function(src_excel_file, dest_dir, head) {
            ...
            for (var i_row = head; i_row < sheet.maxRow; i_row++) {
                ...
                for (var i_col = 0; i_col < col_type.length; i_col++) {
                    ...
                    switch (type) {
                        ...
                        case 'string':
                            if (cell) {
                                row_obj[col_name[i_col]] = cell.value.toString();
                            }else{
                                row_obj[col_name[i_col]]="";
                            };
                            break;
                        case 'number':
                            //+xxx.toString() +表示转number
                            var isNumber = !isNaN(+cell.value.toString());
                            if (cell&& isNumber) {
                                row_obj[col_name[i_col]] = Number(cell.value);
                            }else{
                                row_obj[col_name[i_col]]=0;
                            };
                            if (!isNumber) {
                        ...
                        case 'bool':
                            if (cell) {
                                row_obj[col_name[i_col]] = toBoolean(cell.value.toString());
                            }else{
                                row_obj[col_name[i_col]]=false;
                            };
                            break;
                        case '{}':
                            if (cell) {
